...
namespace PowrIntegration.BackOfficeService.Powertill;

internal sealed class PurchaseFileExport(IOptions<PowertillOptions> options, IOptions<ZraApiOptions> zraApiOptions, IDbContextFactory<PowrIntegrationDbContext> dbContextFactory)
{
    ...
    private int GetTaxGroupFor(int salesGroupId)
    {
        return
            _zraApiOptions
                .TaxMappings
                .FirstOrDefault(x => x.SalesGroupId == salesGroupId)
                ?.TaxGroupId
                ?? 1;
    }
}
